...
mod tests {
    ...
    async fn test_migrations_create_schema() {
        ...
        let pool = run_migrations(db_url)
            .await
        ...
        assert!(tables.contains(&"events".to_string()), "events table missing");
        assert!(tables.contains(&"rewards".to_string()), "rewards table missing");
        assert!(tables.contains(&"jobs".to_string()), "jobs table missing");
        assert!(tables.contains(&"agent_runs".to_string()), "agent_runs table missing");
        assert!(tables.contains(&"rag_chunks".to_string()), "rag_chunks table missing");
