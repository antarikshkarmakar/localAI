...
## Batch: 15 papers + KDA link (2026-08)

Deep-read the four that could change the design; the rest triaged by abstract.

| Paper | Verdict |
|---|---|
| **SAGE: Self-Evolving Agentic Graph-Memory** (2605.12061) | ★★ **ADOPTED** → spec 02 M11h. Names a real hole: our `kg_*` graph is written once at distill and read forever — nothing downstream improves it. Their point is that a graph used as *static retrieval middleware* can't recover full evidence chains from partial cues and never learns from use. Two feedback edges specced: reader→writer (retrieval success/failure updates the graph; failure is a signal ABOUT the graph) and partial-cue chain recovery (walk fragment → entity → facts → source chunks, which is what makes the CoE audit L1b checkable). Deferred to when retrieval exists — a feedback loop over an unbuilt retrieval path would be speculative. |
| **CoMem / "Understanding Is Done Early"** (2607.28263) | ★ **WATCH — mechanism, not adoptable now.** Exploits a depth division of labor: semantics resolve in lower/middle layers, so write each context chunk through an *intermediate* layer, cache the residual tensors, and recompute only the query-conditioned upper layers. Read compute/memory then become **independent of stored-context length** — the same "long context without KV blowup" prize as KDA, reached differently. Blockers: needs layer-level access + a continued-trained LoRA (Qwen3-8B), i.e. it lives below llama.cpp's API (ADR-004's line). Re-open only if this lands upstream in llama.cpp. |
| **Argus: Agentic Runtime for Long-Horizon Reasoning** (2608.05144) | ★★ **VALIDATES, one idea worth taking.** Their thesis — "unrestricted pivoting is indistinguishable from rationalized failure, so each material change must be evidence-backed, role-gated, and recorded" — is precisely our self-mod chain (S10 council+canary, L10e change manifest, ledger). Independent convergence on the same safety shape. The distinct idea: **persist vs pivot as an explicit runtime decision** with evidence gating, rather than an agent silently thrashing between approaches. Candidate for the repair ladder (spec 09) — right now the ladder only climbs; it has no "this whole route is wrong, change approach" rung. Decide-by Phase 6. |
| **Test-Time Scaling: Regimes, Evaluation, Reproducibility** (2608.04001) | ★ **Reinforces R1b, does not overturn it.** Its contribution is that "test-time scaling" now names several *different* regimes that get conflated in benchmarks — consistent with our sample-and-select correction (spec 06 R1b) being regime-dependent rather than universal. Keep as the reference for how to *measure* effort/route choices on the frozen eval set before trusting them (spec 14). No spec change. |

Loop-4 / training cluster (all DGX-era, none actionable pre-hardware): **AgentOpsD** recursive self-distillation for agentic RL (2608.05987), **On-Policy Self-Distillation without Any Supervision**, **GiGPO** group-in-group policy optimization (2505.10978), **Categorical Critics for LLM RL** (2608.02181), **Meta-RL Induces Exploration in Language Agents** (2512.16848). Filed with the existing Phase-9 method bake-off (ReOPD / KTO / Weak-to-Strong). Meta-RL is the most interesting of the set — exploration is our L10f weak point.

**Self-Evolving Coding Agents** (2608.03392) — same family as Living-Harness/Harness-R1, already adopted via spec 09 H1c + spec 10; revisit at Phase 6 if it adds beyond those.
