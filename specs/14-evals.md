...
| `memory_ops` | memory-system efficiency (Mercury/MemGPT operational-benchmark class) | KPI-08 adjunct | (a) **context_efficiency**: tokens injected to restore working state for a fixed task set — trend must not grow as KB grows (bloat alarm); (b) **duplicate_memory_rate**: fraction of near-duplicate chunks/facts AND preference records (spec 02 §5.1) stored (shingle overlap) — dedup (D4, M1 content-hash, M12d merge-on-duplicate) should hold it near zero; (c) resurfacing precision once proactive injection lands (open-questions, Phase 4): injected reminders judged relevant vs noise; (d) **preference_set_size**: preference records stay in the dozens, not thousands (Pachaar "curated beats accumulated" — unbounded growth is an alarm, M12d audit failing) |
