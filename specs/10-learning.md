...
- **L1b — Chain-of-Evidence, phantom-reference check (Google Science One / ScientistOne):** every extracted claim carries a **structured evidence chain** — `claim → source_chunk_id (spec 02 kg_facts) → resolvable citation`. The fact-check (spec 05 mode 3) runs a **phantom-reference audit**: for each cited source, verify the source *actually contains* support for the claim (not just that the URL resolves). A claim whose cited chunk doesn't support it is rejected as a fabricated citation — the distiller hallucinated the linkage. Baselines in that work hallucinated up to 21% of references; the check is the guard. This makes "citation resolves" (spec 06 R8) stricter: resolves AND substantiates. Ties the audit to `kg_facts.source_chunk_id` which already exists.
