...
- **H1c — Repair state-graph (Living-Harness, arXiv 2607.26598):** H1b digests don't just feed a flat observation list — they build a **directed repair graph** in `kg_*` (spec 02 §4.3): nodes = situations `(error_class, failure_category, implicated_component)`, edges = repairs that were tried, edge weights = `(success_rate, cost, rungs_climbed)` accumulated across episodes. On a new failure, the healer **retrieves the matching node** and takes the highest-weight repair edge first, instead of always climbing the linear ladder. Key property (their result): the graph is **retrieval-only reusable across model backbones** — repairs learned on the frozen Gemma transfer to a future DGX-trained model without retraining, because the graph records *what worked in what situation*, not model weights. Tools/context stay frozen; only this procedural repair-knowledge accumulates. Bounds unchanged: cost/depth caps (H2) always hold; a graph edge is advisory routing, never a gate.
