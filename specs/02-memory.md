...
- **M3b — Observation masking, the zero-cost tier (arXiv 2606.00408):** for **tool-call observations** specifically, cleanup is tiered: FIRST mask, THEN summarize. Stale observations beyond the K most recent (config) are replaced with a placeholder retaining `(tool, args)` but dropping the content — no model call, unlike M2/M3 summarization which costs a generation on the single-flight queue (spec 03 I1). Rules: (a) **error observations are NEVER masked** — the repair ladder (spec 09) reads captured errors from context; a masked error blinds rung 1; (b) masked content is recoverable — the placeholder carries the ledger event id, so the agent can re-fetch if it decides the observation matters after all; (c) apply masks only when overflow forces context reassembly anyway, or at focus boundaries — mid-context rewrites kill the KV prefix cache (spec 03 I2b). **Regime note:** masking's gain is capacity-dependent (inverted-U — peaks at mid-capacity models, collapses on saturated ones); 12B/E4B sits at the peak. Re-check if ADR-003 ever lands a much stronger model.
...
- **M11g — Reconstruct, don't replay (MemHarness arXiv 2607.28272; third signal after Mercury + Proactive-Memory):** retrieved episodes/facts are NOT injected verbatim as static records — they are **reconstructed for the current context**: summarized/re-framed to the active goal, stale specifics dropped, only the transferable lesson kept. Verbatim replay bloats context and drags irrelevant specifics; reconstruction is what makes past experience *reusable* rather than *repeated*. This is why M2/M3 summarize (not truncate) and why episodes store a ≤300-token summary, not the raw transcript — the philosophy now has a name and three independent confirmations.
...
- **M11i — Write-path filtering is a first-class stage (agent-memory survey, arXiv 2603.07670).** The survey's framing is `write → manage → read`, and it flags the **write path** as where deployed systems actually fail: everything gets stored, so the store degrades and recall precision falls. Our design has an explicit read path (M11 hybrid retrieval), an explicit manage path (M7 lifecycle, M11c invalidation, M6b archival) — but the write path is currently "distill produces it, we store it". Missing filter, to add with the distiller's OKF commit: reject a candidate fact that is (a) already asserted by an existing non-invalidated `kg_fact` with the same subject+predicate (duplicate, not new knowledge), (b) below a confidence floor, or (c) unfalsifiable phrasing with no resolvable citation (fails L1b's chain-of-evidence check anyway). Cheaper to refuse at write than to prune later, and it directly serves the `duplicate_memory_rate` metric (spec 14 memory_ops).
- **M11h — The graph is a substrate, not read-only middleware (SAGE, arXiv 2605.12061).** Our `kg_*` layer is currently write-once-at-distill then read-forever: nothing downstream ever improves it. SAGE's finding is that treating the memory graph as static retrieval middleware caps what it can do — it cannot recover a full evidence chain from a partial cue, and it never learns from how it was used. Two feedback edges to add when retrieval goes live:
  1. **Reader → writer feedback.** When a retrieval succeeds or fails to ground an answer, that outcome updates the graph — edges that repeatedly carry useful evidence strengthen; entities that never resolve get merged or pruned (ties M11e resolution and the L2 source-quality score). Retrieval failure is a *signal about the graph*, not just a bad answer.
  2. **Partial-cue chain recovery.** Retrieval should walk from whatever fragment the query names to the *complete* evidence chain (entity → facts → source chunks), rather than returning only the directly-matching chunk. This is what makes the CoE audit (L1b) checkable end-to-end.
  Deliberately deferred until embeddings + retrieval exist — a feedback loop over a retrieval path that isn't built yet would be speculative.
