...
- **D7c — alphaXiv as a relevance signal, not a second source of papers.** alphaXiv mirrors arXiv ids and layers community discussion on top. Consequences the pipeline must respect:
  1. **Same paper universe → dedup by arXiv id, not by URL.** A paper reachable at both `arxiv.org/abs/<id>` and `alphaxiv.org/abs/<id>` is ONE paper; fetching and distilling it twice wastes the loop and double-counts it in the KB. Content-hash dedup (D4) catches identical bytes but the two pages differ (one carries comments), so the id is the dedup key.
  2. **The commentary is the added value, and it is a different KIND of claim.** Community discussion says *what people think matters* — a curation signal for the gap-directed filter (D7b rule 1), and a useful counterweight to reading purely by category volume. It is opinion, not result: distilled discussion is `UnverifiedKb` and must never be conflated with the paper's own claims. Store it linked to the paper's facts (`kg_facts` via `source_chunk_id`), not merged into them.
  3. Both hosts need explicit allowlist entries (D1); neither is implicitly trusted for being academic.
