...
- **R1b — LOCAL_SELFCHECK = sample-and-SELECT, not self-critique (corrected by arXiv 2607.28576 "Sample More, Reflect Less"):** primary path is **Best-of-N + objective selection** — draw k samples (serial on CPU anyway, I1) and pick by a *verifier* (tests/compile/citation-resolve where one exists) or self-consistency vote where none does. Rigorous finding at 1.5B–7B (brackets our E4B): at equal token cost, repeated sampling beats self-refine and reflexion; **self-critique-then-rewrite is reliably worse at small scale** (a small model judging its own output tends to rubber-stamp it). **Selection is safe; self-inspection-rewrite is the harmful part.** So: *pick among samples, don't have the small model criticize-and-rewrite.*
  - **PDR (Parallel-Distill-Refine, arXiv 2510.01123) is scale-gated:** the distill→refine variant only earns its keep on a large enough model (DGX-era 12B+ resident). Until then, default to Best-of-N+verifier. Re-enable PDR-refine when the fast model crosses the scale where self-inspection stops hurting (measure on the frozen eval set, spec 14, before switching).
